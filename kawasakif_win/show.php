...
<!-- ■■■■ セクション ■■■■ -->
<?php for ($i=0; $i < count($kawasakif_winmenus); $i++): ?>
  <?php if ($_GET['name'] == $kawasakif_winmenus[$i]->getName()):?>
    <section class="section">
      <div class="section_IMG">
        <p class="image_show"><img src="../<?php echo $kawasakif_winmenus[$i] -> getImage() ?>"></p>
      </div>
      <div class="section_contents">
        <p><img src="../image/kawasakif.png"><span>川崎フロンターレ優勝記念グッズ</span></p>
        <p><?php echo $kawasakif_winmenus[$i]->getName() ?></p>
        <p>価格:<span><?php echo $kawasakif_winmenus[$i]->getTaxPrice() ?>円</span>(税込み)</p>
        <p>個数:<select class="select" name=" <?php echo $kawasakif_winmenus[$i]->getName() ?>">
...
<h2>他のおすすめ商品</h2>
<?php foreach($kawasakif_winmenus as $kawasakif_winmenu): ?>
  <section class="recommended">
    <p><a href="show.php?name=<?php echo $kawasakif_winmenu->getName() ?>"></p>
    <p class="image_css"><img src="../<?php echo $kawasakif_winmenu -> getImage() ?>"></p>
    <p class="menu_css"><?php echo $kawasakif_winmenu->getName() ?></p>
    <p class="price_css"><?php echo $kawasakif_winmenu->getTaxPrice() ?><span>円(税込み)</span></p></a>
  </section>
<?php endforeach ?>
</div>
</div>
